...
class Visualizer3D:
    ...
    def _build_hud_payload(self) -> dict | None:
    ...
    def update(
        ...
        active_turn and hud state are embedded in the message so the viewer
        syncs both the turn indicator and HUD with the animation queue rather
        than updating them immediately.
        """
        hud = self._build_hud_payload()
        if transition_id is not None:
            ...
            self.server.send_position(node_id, turn=active_turn, hud=hud)
