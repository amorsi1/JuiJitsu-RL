...
from .buffers import MaskableRecurrentRolloutBuffer
...
class MaskableRecurrentPPO(RecurrentPPO):
    """
    RecurrentPPO with invalid-action masking.

    Combines the LSTM-based rollout of RecurrentPPO with the action-masking
    infrastructure of MaskablePPO. The masking affects only the action distribution
    layer; LSTM memory operates on the full temporal context independently.
    """
    ...
    def _setup_model(self) -> None:
        ...
        # Use MaskableRecurrentRolloutBuffer instead of the base RecurrentRolloutBuffer
    ...
    def collect_rollouts(
        ...
        assert isinstance(rollout_buffer, MaskableRecurrentRolloutBuffer), (
            "rollout_buffer must be a MaskableRecurrentRolloutBuffer"
        )
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            continue_training = self.collect_rollouts(
                self.env, callback, self.rollout_buffer, self.n_steps, use_masking=use_masking
            )
            if not continue_training:
                break
